...
# 입력  : action_resnet_mt_8l.onnx (5 outputs: action_upper/lower/pose/hand/foot)
# 출력  : action_resnet_mt_8l.hef
#
# 사전조건:
...
# 산출물 정리
for f in action_resnet_mt_8l.har action_resnet_mt_quantized.har \
         action_resnet_mt_8l.hef action_resnet_mt_compiled.har; do
    rm -f "$f"
done
...
hailo parser onnx "$ONNX" \
    --hw-arch "$HW_ARCH" \
    --har-path action_resnet_mt_8l.har \
    --tensor-shapes "input=[1,7,60,25]"

echo ""
echo "=== 2/3. Optimize — 1500 calibration ==="
hailo optimize action_resnet_mt_8l.har \
...
if [ -f action_resnet_mt_quantized.hef ]; then
    mv action_resnet_mt_quantized.hef action_resnet_mt_8l.hef
fi

echo ""
echo "=== 완료 ==="
ls -lh action_resnet_mt_8l.hef
