...
from smutils.bbox.iou_calculator import bbox_overlaps
from smtrack.models.trackers.base_tracker import BaseTracker

class ByteTrackerMod(BaseTracker):
    ...
    def init_track(self, id, obj):
        ...
        bbox = self.tracks[id].bboxes[-1][:, :4]
        assert bbox.ndim == 2 and bbox.shape[0] == 1
        bbox = bbox.squeeze(0).cpu().numpy()
        self.tracks[id].mean, self.tracks[id].valid  = self.of.initiate(bbox)
    ...
    def update_track(self, id, obj):
        ...
        if self.tracks[id].tentative:
            if len(self.tracks[id]['bboxes']) >= self.num_tentatives:
                self.tracks[id].tentative = False
        bbox = self.tracks[id].bboxes[-1]#[:, :4]  # size = (1, 4)
        assert bbox.ndim == 2 and bbox.shape[0] == 1
        bbox = bbox.squeeze(0).cpu().numpy()
        ...
        self.tracks[id].mean = self.of.update(self.tracks[id].mean, bbox)
    ...
    def pop_invalid_tracks(self, frame_id):
        ...
        for k, v in self.tracks.items():
            ...
            case2 = v.tentative and frame_id - v['frame_ids'][-1] >= self.num_frames_retain//3

            case3 = v.valid and frame_id - v['frame_ids'][-1] >= self.num_frames_retain//3
    ...
    def track(self,
        ...
        self.of.preprocessing(img, mask)
        for id in self.tracks.keys():   
            self.tracks[id].mean, self.tracks[id].valid = self.of.predict(self.tracks[id].mean)
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            # 1. use optical flow to predict current location
            # for id in self.tracks.keys():   
            #     self.tracks[id].mean = self.of.predict(self.tracks[id].mean)
            ...
            for i, id in enumerate(self.confirmed_ids):
                ...
                case_2 = True#self.tracks[id].frame_ids[-1] == frame_id - 1
